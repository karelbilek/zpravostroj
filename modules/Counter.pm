...
sub count_TMT {
	...
	sub count_node {
		...
		if ($node->get_deref_attr('m.rf')) {
			...
			add(\%named_entities, $name) if 
					((my $type=($node->get_attr('ne_type'))) 
				and 
					(length(my $name = $node->get_attr('normalized_name'))>3) 
				and 
					($type =~ /^(g|m|q|P|ps)/));
			...
			if (my $lemma_b = (make_normal_word($lemma))) { 
				...
				push (@last_words, [$lemma_b, $lost_forms.$form]);
				$lost_forms = "";
				...
				if (is_word($node->get_attr('form'))) {
					#"a", "nebo", "který" ...
					$lostforms=$lost_forms.$node->get_attr('form')." ";		
				}
			}
		}
	    if (my @children = ($node->get_children)) {
	        map (coun_node($_), @children);
	    }
	}
